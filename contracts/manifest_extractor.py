...
def get_system_tag(system_name, namespace=None):
    ...
    """
    base_name = f"{'zidle' if namespace is None else namespace}-{system_name}"
    return base_name
...
def extract_system_addresses(json_data, namespace=None):
    ...
    target_systems = {
        'character_minter': get_system_tag('character_minter', namespace),
        'character': get_system_tag('character', namespace),
    ...
    command = (
        f"starkli invoke \\\n"
        f"    <contract_address> \\\n"
        f"    selector:update_minter_role \\\n"
        f"    {addresses['character_minter']} \\\n"
        f"    --watch \n"
    )
