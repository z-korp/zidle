...
const GoalsTab: React.FC<GoalsTabProps> = ({ debugMode = false }) => {
  ...
  const extractedActions = useMemo(() => {
    return messages
      ...
      .map(m => ({
        ...
        type: m.type,
        description: typeof m.data?.description === 'string' ? m.data.description : 
  ...
  const extractedGoals = useMemo(() => {
    const goalMessages = messages.filter(m => 
      m.type === 'goal_created' || 
      m.type === 'goal_updated' || 
    ...
    const goals = goalMessages.map(m => ({
      ...
      description: typeof m.data?.description === 'string' ? m.data.description :
                  typeof m.message === 'string' ? m.message :
                  'Unknown goal',
      status: m.type === 'goal_completed' ? 'completed' : 
             m.type === 'goal_created' ? 'active' : 
             'pending',
      progress: 0,
      timestamp: m.timestamp,
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div>
                      ...
                      <Progress value={Number(goal.progress)} className="w-full" />
                    </div>
